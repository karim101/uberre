...
class PropertyController extends AppController
{
    ...
    public function addProperty()
    {
		...
		$ServiceCost = TableRegistry::get('ServiceCosts');
				...
				if($service_value['id']!=5){
					...
					curl_close ($ch);
					 if(!empty($result)){
						...
						{
						/*****************  Add property in main table   ******************************/
								if($main_property_id ==0)
								{
									...
									$main_property = $property->save($property_to_insert);
									$main_property_id = $main_property['id'];
								}	
							/***************  Multiple service name from our database  **************************/
							foreach($service_value['service_file'] as $service_multiple){
								$properties_details = $ServiceCost->find('all',array('conditions'=>array('id' =>$service_multiple['service_cost_id']),'fields'=>array('type','id')))->first();
								if(!empty($main_data->items)){
									foreach($main_data->items as $item_type){
										$name = explode("UberRE - ",$item_type->name);
										$type =$name['1'];
										if($type==$properties_details['type']){
											/*************************** We are getting ServiceId and  SupplierId  from API *************************************************/
											$this->request->data['ubookrServiceId']=$item_type->id;
											$this->request->data['ubookrServiceTypeId']=$item_type->serviceType->id;
											$this->request->data['supplierId']=$item_type->supplierId;
											/************************************************** End *************************************************************************/
											$this->request->data['property_id'] =$main_property_id;
